    ...
    public boolean process(final Set<? extends TypeElement> set, final RoundEnvironment roundEnvironment)
    {
        ...
        for (final Element aeronicElement : aeronicElements)
        {
            ...
            try
            {
                ...
                final JavaFileObject invokerSourceFile = processingEnv.getFiler()
                    .createSourceFile(invokerPath, aeronicElement);
                final Writer writer = invokerSourceFile.openWriter();
                writer.append(invokerSource);
                writer.close();
                processingEnv.getMessager()
                    .printMessage(Diagnostic.Kind.NOTE, "Written file for %s".formatted(invokerPath));
            }
            catch (final
            IOException e)
            {
            ...
            try
            {
                ...
                final JavaFileObject publisherSourceFile = processingEnv.getFiler()
                    .createSourceFile(publisherPath, aeronicElement);
                final Writer writer = publisherSourceFile.openWriter();
                writer.append(publisherSource);
                writer.close();
                processingEnv.getMessager()
                    .printMessage(Diagnostic.Kind.NOTE, "Written file for %s".formatted(publisherPath));
            }
            catch (final
            IOException e)
            {
